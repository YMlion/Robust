...
class AutoPatchTransform extends Transform implements Plugin<Project> {
    ...
    def initConfig() {
        ...
        Config.robustGenerateDirectory = "${project.buildDir}" + File.separator +
            ...
            File.separator
        dex2SmaliCommand = "  java -jar ${baksmaliFilePath} -o classout" + File.separator +
            "  $Constants.CLASSES_DEX_NAME"
        smali2DexCommand = "   java -jar ${smaliFilePath} classout" + File.separator +
            " -o " +
            Constants.PATACH_DEX_NAME
        jar2DexCommand =
            "   java -jar ${dxFilePath} --dex --output=$Constants.CLASSES_DEX_NAME  " + Constants.ZIP_FILE_NAME
    ...
    static def copyJarToRobust() {
        ...
        for (String libName : Constants.LIB_NAME_ARRAY) {
            ...
            if (inputStream == null) {
                System.out.println("Warning!!!  Did not find " + libName +
                    " ，you must add it to your project's libs ")
                continue
            }
    ...
    def autoPatch(List<CtClass> box) {
        ...
        String patchPath = buildDir.getAbsolutePath() + File.separator +
            Constants.ROBUST_GENERATE_DIRECTORY +
            File.separator
    ...
    def zipPatchClassesFile() {
        ZipOutputStream zipOut = new ZipOutputStream(
            new FileOutputStream(Config.robustGenerateDirectory + Constants.ZIP_FILE_NAME))
        zipAllPatchClasses(Config.robustGenerateDirectory + Config.patchPackageName.substring(0,
            Config.patchPackageName.indexOf(".")), "", zipOut)
        zipOut.close()
    }
    ...
    def generatePatch(List<CtClass> box, String patchPath) {
        if (!Config.isManual) {
            ...
            for (String fullClassName : Config.modifiedClassNameList) {
                ...
                CtClass patchClass = PatchesFactory.createPatch(patchPath, ctClass, false,
                    NameManger.getInstance().getPatchName(ctClass.name),
            ...
            if (Config.methodNeedPatchSet.size() > 0) {
                throw new RuntimeException(
                    " some methods haven't patched,see unpatched method list : " + Config.methodNeedPatchSet.
                        toListString())
            }
        } else {
    ...
    def deleteTmpFiles() {
        ...
        if (!directory.isDirectory()) {
            throw new RuntimeException(
                "patch directory " + Config.robustGenerateDirectory + " does not exist")
        } else {
            directory.listFiles(new FilenameFilter() {
                @Override
                boolean accept(File file, String s) {
                    return !(Constants.PATACH_JAR_NAME == s)
                }
            }).each {
                if (it.isDirectory()) {
                    it.deleteDir()
    ...
    def initSuperMethodInClass(List originClassList) {
        ...
        for (String modifiedFullClassName : originClassList) {
            List<CtMethod> invokeSuperMethodList = Config.invokeSuperMethodMap.getOrDefault(
                modifiedFullClassName, new ArrayList())
            ...
            modifiedCtClass.declaredMethods.findAll {
                return Config.patchMethodSignatureSet.contains(
                    it.longName) || InlineClassFactory.allInLineMethodLongname.contains(it.longName)
            }.each { behavior ->

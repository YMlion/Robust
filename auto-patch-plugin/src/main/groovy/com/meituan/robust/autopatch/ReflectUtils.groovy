...
class ReflectUtils {
    ...
    static String setFieldString(CtField field, Map memberMappingInfo, String patchClassName,
        String modifiedClassName) {
        boolean isStatic = isStatic(field.modifiers)
        StringBuilder stringBuilder = new StringBuilder("{")
        if (isStatic) {
            println("setFieldString static field " + field.getName() +
                "  declaringClass   " +
            ...
            if (AccessFlag.isPublic(field.modifiers)) {
                ...
                if (field.declaringClass.name.equals(patchClassName)) {
                    stringBuilder.append(Constants.ROBUST_UTILS_FULL_NAME +
                        ".setStaticFieldValue(\"" +
                        getMappingValue(field.name, memberMappingInfo) +
                        "\"," +
                        modifiedClassName +
                        ".class,\$1);")
                } else {
            ...
            if (Constants.isLogging) {
                stringBuilder.append(
                    "  android.util.Log.d(\"robust\",\"set static  value is \" +\"" + (field.getName()) +
                        " ${getCoutNumber()}\");")
            }
        } else {
            ...
            stringBuilder.append(
                "instance=((" + patchClassName + ")\$0)." + Constants.ORIGINCLASS + ";")
            stringBuilder.append("}else{")
            stringBuilder.append("instance=\$0;")
            stringBuilder.append("}")
            stringBuilder.append(Constants.ROBUST_UTILS_FULL_NAME + ".setFieldValue(\"" +
                getMappingValue(field.name, memberMappingInfo) +
                "\",instance,\$1,${field.declaringClass.name}.class);")
            if (Constants.isLogging) {
                stringBuilder.append(
                    "  android.util.Log.d(\"robust\",\"set value is \" + \"" + (field.getName()) +
                        "    ${getCoutNumber()}\");")
            }
        }
        stringBuilder.append("}")
    ...
    static String getFieldString(CtField field, Map memberMappingInfo, String patchClassName,
        String modifiedClassName) {
        ...
        if (isStatic) {
            if (AccessFlag.isPublic(field.modifiers)) {
                ...
                if (INLINE_R_FILE && isRFile(field.declaringClass.name)) {
                    println("getFieldString static field " + field.getName() +
                        "   is R file macthed   " +
                        field.declaringClass.name)
                ...
                if (field.declaringClass.name == patchClassName) {
                    stringBuilder.append("\$_=(\$r) " + Constants.ROBUST_UTILS_FULL_NAME +
                        ".getStaticFieldValue(\"" +
                        getMappingValue(field.name, memberMappingInfo) +
                        "\"," +
                        modifiedClassName +
            ...
            if (Constants.isLogging) {
                stringBuilder.append(
                    "  android.util.Log.d(\"robust\",\"get static  value is \" +\"" + (field.getName()) +
                        "    ${getCoutNumber()}\");")
            }
        } else {
            stringBuilder.append("java.lang.Object instance;")
            ...
            stringBuilder.append("\$_=(\$r) " + Constants.ROBUST_UTILS_FULL_NAME +
                ".getFieldValue(\"" +
                getMappingValue(field.name, memberMappingInfo) +
                "\",instance,${field.declaringClass.name}.class);")
            if (Constants.isLogging) {
                stringBuilder.append(
                    "  android.util.Log.d(\"robust\",\"get value is \" +\"" + (field.getName()) +
                        "    ${getCoutNumber()}\");")
            }
        }
        stringBuilder.append("}")
    ...
    static String getParameterClassSignature(String signature, String patchClassName) {
        ...
        for (int index = 1; index < signature.indexOf(")"); index++) {
            if (Constants.OBJECT_TYPE == signature.charAt(index) &&
                signature.indexOf(Constants.PACKNAME_END) !=
                -1) {
                name = signature.substring(index + 1,
                    signature.indexOf(Constants.PACKNAME_END, index)).replaceAll("/", ".")
                if (name == patchClassName) {
            ...
            if (Constants.ARRAY_TYPE == String.valueOf(signature.charAt(index))) {
                isArray = true
            }
        }
        if (signureBuilder.length() > 0 &&
            String.valueOf(signureBuilder.charAt(signureBuilder.length() - 1)) ==
            ",") {
    ...
    static String getCreateClassString(NewExpr e, String className, String patchClassName,
        boolean isStatic) {
        StringBuilder stringBuilder = new StringBuilder()
        if (e.signature == null) {
            return "{\$_=(\$r)\$proceed(\$\$);}"
        ...
        if (isStatic) {
            ...
            if (signatureBuilder.length() > 1) {
                stringBuilder.append(
                    "java.lang.Object parameters[]=" + Constants.GET_REAL_PARAMETER + "(\$args);")
                if (Constants.isLogging) {
                    stringBuilder.append(
                        "  android.util.Log.d(\"robust\",\" parameters[] from method     ${getCoutNumber()} \"+parameters);")
                }
    ...
    static String getNewInnerClassString(String signature, String patchClassName, boolean isStatic,
        String className) {
        StringBuilder stringBuilder = new StringBuilder()
        stringBuilder.append("{")
        ...
        if (isStatic) {
            ...
            if (signatureBuilder.length() > 1) {
                if (Constants.isLogging) {
                    stringBuilder.append(
                        "  android.util.Log.d(\"robust\",\"  inner Class new     ${getCoutNumber()}\");")
                }
                stringBuilder.append(
                    "java.lang.Object parameters[]=" + Constants.GET_REAL_PARAMETER + "(\$args);")
                stringBuilder.append("\$_= (\$r)" + Constants.ROBUST_UTILS_FULL_NAME +
    ...
    static String getMethodCallString(MethodCall methodCall, Map memberMappingInfo,
        CtClass patchClass, boolean isInStaticMethod, boolean inline) {
        ...
        if (isStatic(methodCall.method.modifiers)) {
            if (isInStaticMethod) {
                ...
                if (AccessFlag.isPublic(methodCall.method.modifiers)) {
                    ...
                    if (signatureBuilder.toString().length() > 1) {
                        stringBuilder.append("\$_=(\$r) " + Constants.ROBUST_UTILS_FULL_NAME +
                            ".invokeReflectStaticMethod(\"" +
                            getMappingValue(getJavaMethodSignureWithReturnType(methodCall.method),
                                memberMappingInfo) +
                            "\"," +
                            methodCall.method.declaringClass.name +
                ...
                if (Constants.isLogging) {
                    stringBuilder.append(
                        "  android.util.Log.d(\"robust\",\"invoke static  method is      ${getCoutNumber()}  \" +\"" +
            ...
            if (!isInStaticMethod) {
                ...
                stringBuilder.append(
                    "instance=((" + patchClass.getName() + ")\$0)." + Constants.ORIGINCLASS + ";")
                stringBuilder.append("}else{")
                stringBuilder.append("instance=\$0;")
                stringBuilder.append("}")
                if (signatureBuilder.toString().length() > 1) {
                    stringBuilder.append(
                        "java.lang.Object parameters[]=" + Constants.GET_REAL_PARAMETER +
                            "(\$args);")
                    stringBuilder.append("\$_=(\$r) " + Constants.ROBUST_UTILS_FULL_NAME +
                        ".invokeReflectMethod(\"" +
                        getMappingValue(getJavaMethodSignureWithReturnType(methodCall.method),
                            memberMappingInfo) +
                        "\",instance,parameters,new Class[]{" +
                        signatureBuilder.toString() +
                        "},${methodCall.method.declaringClass.name}.class);")
                } else {
                    stringBuilder.append("\$_=(\$r)" + Constants.ROBUST_UTILS_FULL_NAME +
                        ".invokeReflectMethod(\"" +
                        getMappingValue(getJavaMethodSignureWithReturnType(methodCall.method),
                            memberMappingInfo) +
                        "\",instance,\$args,null,${methodCall.method.declaringClass.name}.class);")
                }
                if (Constants.isLogging) {
                    stringBuilder.append(
                        "  android.util.Log.d(\"robust\",\"invoke  method is      ${getCoutNumber()} \" +\"" +
                            methodCall.methodName +
                            "\");")
                }
            } else {
    ...
    static String getCastString(Cast c, CtClass patchClass) {
        ...
        stringBuilder.append(
            "\$_=((" + patchClass.getName() + ")\$1)." + Constants.ORIGINCLASS + ";")
        stringBuilder.append("}else{")
    ...
    static String getInLineMemberString(CtMethod method, boolean isInStaticMethod,
        boolean isNewClass) {
        ...
        if (NameManger.getInstance().
            getPatchNameMap().
            get(NameManger.getInstance().
                getInlinePatchNameWithoutRecord(method.declaringClass.name)) != null) {
            if (Constants.isLogging) {
                stringBuilder.append(
                    "  android.util.Log.d(\"robust\",\"deal inline in first   ${getCoutNumber()}  \" +\"" +
                        method.name +
                        "\");")
            }
            stringBuilder.append(
                NameManger.getInstance().getInlinePatchName(method.declaringClass.name) +
                    " instance;")
            if (Constants.isLogging) {
                stringBuilder.append(
                    "  android.util.Log.d(\"robust\",\"deal inline method after new instance   ${getCoutNumber()}    \" +\"" +
                        method.name +
                        "\");")
            }
            if (isInStaticMethod || isNewClass) {
                //在static method中不需要考虑参数为this的问题
                stringBuilder.append(" instance=new " + NameManger.getInstance().
                    getInlinePatchName(method.declaringClass.name) + "(\$0);")
                if (!isStatic(method.modifiers)) {
                    stringBuilder.append("\$_=(\$r)instance." + getInLineMethodName(method) +
                        "(" +
                        parameterBuilder.toString() +
                        ");")
                } else {
                    stringBuilder.append("\$_ = (\$r)" + NameManger.getInstance().
                        getInlinePatchName(method.declaringClass.name) +
                        "." +
                        getInLineMethodName(method) +
                        "(" +
                        parameterBuilder.toString() +
                        ");")
                }
                ...
                stringBuilder.append("java.lang.Object target[]=" + Constants.GET_REAL_PARAMETER +
                    "(new java.lang.Object[]{\$0});")
                stringBuilder.append(" instance=new " + NameManger.getInstance().
                    getInlinePatchName(method.declaringClass.name) + "(target[0]);")
                ...
                if (!isStatic(method.modifiers)) {
                    ...
                    if (signatureBuilder.toString().length() > 1) {
                        ...
                        stringBuilder.append("\$_=(\$r) " + Constants.ROBUST_UTILS_FULL_NAME +
                            ...
                            getInLineMethodName(method) +
                            "\"," +
                            NameManger.getInstance().
                                getInlinePatchNameWithoutRecord(method.declaringClass.name) +
                            ".class,parameters,new Class[]{" +
                            signatureBuilder.toString() +
                            "});")
                    } else {
            ...
            throw new RuntimeException(
                "getInLineMemberString cannot find inline class ,origin class is  " + method.declaringClass.name)
        }
        if (Constants.isLogging) {
            stringBuilder.append(
                "  android.util.Log.d(\"robust\",\"deal inline method   ${getCoutNumber()}   \" +\"" +
                    method.name +
                    "\");")
        }
        stringBuilder.append("}")
    ...
    @Deprecated
    static String invokeSuperString(MethodCall m) {
        StringBuilder stringBuilder = new StringBuilder()

        stringBuilder.append("{")
        if (m.method.returnType != CtClass.voidType) {
            stringBuilder.append("\$_=(\$r)")
        }
        if (m.method.parameterTypes.length > 0) {
            stringBuilder.append(getStaticSuperMethodName(m.methodName) + "(this," +
                Constants.ORIGINCLASS +
                ",\$\$);")
        } else {
            stringBuilder.append(
                getStaticSuperMethodName(m.methodName) + "(this," + Constants.ORIGINCLASS + ");")
        }
    ...
    static String invokeSuperString(MethodCall m, String originClass) {
        StringBuilder stringBuilder = new StringBuilder()
        ...
        if (m.method.parameterTypes.length > 0) {
            if (!originClass.isEmpty()) {
                stringBuilder.append(
                    getStaticSuperMethodName(m.methodName) + "(null," + originClass + ",\$\$);")
            } else {
                stringBuilder.append(getStaticSuperMethodName(m.methodName) + "(this," +
                    Constants.ORIGINCLASS +
                    ",\$\$);")
    ...
    static String getStaticSuperMethodName(String methodName) {
    ...
    def static getMappingValue(String name, Map memberMappingInfo) {
        if (Constants.OBSCURE) {
            ...
            if (value == null || value.length() < 1) {
                ...
                AutoPatchTransform.logger.warn(
                    "Warning  class name  " + name + "   can not find in mapping !! ")

...
class PatchesFactory {
     ...
     * @return
     */
    private CtClass createPatchClass(CtClass modifiedClass, boolean isInline, String patchName,
        Set patchMethodSignureSet, String patchPath)
        ...
        if (patchMethodSignureSet.size() != 0) {
            for (CtMethod method : modifiedClass.getDeclaredMethods()) {
                //新增方法需要保留在补丁类中
                if (!Config.supportProGuard && Config.newlyAddedMethodSet.contains(
                    method.longName)) {
                    continue
                }
                //不是被补丁的方法
                if ((!patchMethodSignureSet.contains(method.getLongName()) || //不是内联并且是新增的方法
                    (!isInline && Config.methodMap.get(
                        modifiedClass.getName() + "." + JavaUtils.getJavaMethodSignure(method)) ==
                        null))) {
        ...
        if (temPatchClass.getDeclaredMethods().length == 0) {
            ...
            throw new RuntimeException(
                "all methods in patch class are deteted,cannot find patchMethod in class " +
                    temPatchClass.getName())
        }

        JavaUtils.addPatchConstruct(temPatchClass, modifiedClass)
        CtMethod reaLParameterMethod = CtMethod.make(JavaUtils.getRealParamtersBody(),
            temPatchClass)
        temPatchClass.addMethod(reaLParameterMethod)

        dealWithSuperMethod(temPatchClass, modifiedClass, patchPath)

        if (Config.supportProGuard && ReadMapping.getInstance().
            getClassMapping(modifiedClass.getName()) == null) {
            throw new RuntimeException(" something wrong with mappingfile ,cannot find  class  " +
                modifiedClass.getName() +
                "   in mapping file")
        }
        List<CtMethod> invokeSuperMethodList = Config.invokeSuperMethodMap.getOrDefault(
            modifiedClass.getName(), new ArrayList<>())
        ...
        for (CtMethod method : temPatchClass.getDeclaredMethods()) {
            ...
            if (!Config.addedSuperMethodList.contains(method) && reaLParameterMethod !=
                method &&
                !method.getName().startsWith(Constants.ROBUST_PUBLIC_SUFFIX)) {
                method.instrument(new ExprEditor() {
                    void edit(FieldAccess f) throws CannotCompileException {
                        if (Config.newlyAddedClassNameList.contains(f.getClassName())) {
                            return
                        }
                        Map memberMappingInfo = getClassMappingInfo(
                            f.getField().declaringClass.name)
                        try {
                            if (f.isReader()) {
                                f.replace(
                                    ReflectUtils.getFieldString(f.getField(), memberMappingInfo,
                                        temPatchClass.getName(), modifiedClass.getName()))
                            } else if (f.isWriter()) {
                                f.replace(
                                    ReflectUtils.setFieldString(f.getField(), memberMappingInfo,
                                        temPatchClass.getName(), modifiedClass.getName()))
                            }
                    ...
                    void edit(NewExpr e) throws CannotCompileException {
                        ...
                        if (Config.newlyAddedClassNameList.contains(
                            e.getClassName()) || Config.noNeedReflectClassSet.contains(
                            e.getClassName())) {
                            return
                        }

                        try {
                            if (!ReflectUtils.isStatic(
                                Config.classPool.get(e.getClassName()).getModifiers()) && JavaUtils.
                                isInnerClassInModifiedClass(e.getClassName(), modifiedClass)) {
                                e.replace(ReflectUtils.getNewInnerClassString(e.getSignature(),
                                    temPatchClass.getName(), ReflectUtils.isStatic(
                                    Config.classPool.get(e.getClassName()).getModifiers()),
                                    getClassValue(e.getClassName())))
                                return
                            }
                        ...
                        e.replace(
                            ReflectUtils.getCreateClassString(e, getClassValue(e.getClassName()),
                                temPatchClass.getName(),
                                ReflectUtils.isStatic(method.getModifiers())))
                    }
                    ...
                    void edit(Cast c) throws CannotCompileException {
                        ...
                        if (!isStatic) {
                            //inner class in the patched class ,not all inner class
                            if (Config.newlyAddedClassNameList.contains(thisClass.getName()) ||
                                Config.noNeedReflectClassSet.contains(thisClass.getName())) {
                                return
                            }
                    ...
                    void edit(MethodCall m) throws CannotCompileException {
                        ...
                        if (m.getMethodName().contains("lambdaFactory")) {
                            ...
                            m.replace(ReflectUtils.getNewInnerClassString(m.getSignature(),
                                temPatchClass.getName(),
                                ReflectUtils.isStatic(method.getModifiers()),
                                getClassValue(m.getClassName())))
                            return
                        }
                        try {
                            if (!replaceInlineMethod(m, method, false)) {
                                Map memberMappingInfo = getClassMappingInfo(
                                    ...
                                    if (superMethod.getLongName() != null &&
                                        ...
                                        if (ReflectUtils.isStatic(method.getModifiers())) {
                                            ...
                                            if (numberOfLocalVariables > 0) {
                                                ...
                                                firstVariable = methodInfo.getConstPool().
                                ...
                                m.replace(ReflectUtils.getMethodCallString(m, memberMappingInfo,
                                    temPatchClass, ReflectUtils.isStatic(method.getModifiers()),
                                    isInline))
                            }
                        } catch (NotFoundException e) {
                            e.printStackTrace()
     ...
     */
    private
    static CtClass cloneClassWithoutFields(CtClass sourceClass, String patchName,
        List<CtMethod> exceptMethodList) throws NotFoundException, CannotCompileException {
        CtClass targetClass = cloneClass(sourceClass, patchName, exceptMethodList)
        targetClass.declaredFields.each { field -> targetClass.removeField(field)
        }
    ...
    static CtClass cloneClass(CtClass sourceClass, String patchName,
        List<CtMethod> exceptMethodList) throws CannotCompileException, NotFoundException {
    ...
    private void dealWithSuperMethod(CtClass patchClass, CtClass modifiedClass, String patchPath)
        ...
        for (int index = 0; index < invokeSuperMethodList.size(); index++) {
            ...
            if (invokeSuperMethodList.get(index).getParameterTypes().length > 0) {
                methodBuilder.append(
                    "public  static " + invokeSuperMethodList.get(index).getReturnType().getName() +
                        "  " +
                        ReflectUtils.getStaticSuperMethodName(
                            invokeSuperMethodList.get(index).getName()) +
                        "(" +
                        patchClass.getName() +
                        " patchInstance," +
                        modifiedClass.getName() +
                ...
                methodBuilder.append("public  static  " + invokeSuperMethodList.get(index).
                    getReturnType().
                    getName() +
                    "  " +
                    ReflectUtils.getStaticSuperMethodName(
            ...
            if (Constants.isLogging) {
                methodBuilder.append(
                    "android.util.Log.d(\"robust\", \" invoke  " + invokeSuperMethodList.get(index).
                        getLongName() + " staticRobust method \");")
            }
            if (AccessFlag.isPackage(invokeSuperMethodList.get(index).getModifiers())) {
                throw new RuntimeException(
                    "autopatch does not support super method with package accessible ")
            }
            ...
            if (invokeSuperMethodList.get(index).getReturnType() == CtClass.voidType) {
                methodBuilder.append(
                ...
                methodBuilder.append(
                    " return " + NameManger.getInstance().getAssistClassName(patchClass.getName()) +
                        "." +
                        ReflectUtils.getStaticSuperMethodName(
                            invokeSuperMethodList.get(index).getName()) +
                        "(patchInstance,modifiedInstance")
            ...
            methodBuilder.append(JavaUtils.getParameterValue(
                invokeSuperMethodList.get(index).getParameterTypes().length) + ");")
            methodBuilder.append("}")
    ...
    boolean replaceInlineMethod(MethodCall m, CtMethod method, boolean isNewClass)
        throws NotFoundException, CannotCompileException {
        ClassMapping classMapping = ReadMapping.getInstance().
            getClassMapping(m.getMethod().getDeclaringClass().getName())
        if (null != classMapping && classMapping.getMemberMapping().
            get(ReflectUtils.getJavaMethodSignureWithReturnType(m.getMethod())) == null) {
            m.replace(ReflectUtils.getInLineMemberString(m.getMethod(),
                ReflectUtils.isStatic(method.getModifiers()), isNewClass))
            return true
        }
        return false
    }

    static void createPublicMethodForPrivate(CtClass ctClass)
        ...
        for (CtMethod method : privateMethodList) {
            ...
            private2PublicMethod.append("public  " + getMethodStatic(method) +
                " " +
                method.getReturnType().getName() +
                " " +
                Constants.ROBUST_PUBLIC_SUFFIX +
                method.getName() +
                "(" +
                JavaUtils.getParameterSignure(method) +
                "){")
            private2PublicMethod.append("return " + method.getName() +
                "(" +
                JavaUtils.getParameterValue(method.getParameterTypes().length) +
                ");")
    ...
    static CtClass createPatch(String patchpath, CtClass modifiedClass, boolean isInline,
        String patchName, Set patchMethodSignureSet)
        throws NotFoundException, CannotCompileException, IOException {
        return patchesFactory.createPatchClass(modifiedClass, isInline, patchName,
            patchMethodSignureSet, patchpath)
    }
}

...
class InlineClassFactory {
    ...
    static Set getAllInLineMethodLongname() {
    ...
    def dealInLineClass(String patchPath) {
        ...
        for (String fullClassName : inLineClassNameSet) {
            ...
            List<String> inlineMethod = classInLineMethodsMap.getOrDefault(fullClassName,
            ...
            newlyAddClass.declaredMethods.each { method ->
                method.instrument(new ExprEditor() {
                    void edit(MethodCall m) throws CannotCompileException {
                        replaceInlineMethod(m, method, true)
                    }
                })
            }
    ...
    def createHookInlineClass(Set inLineClassNameSet) {
        for (String fullClassName : inLineClassNameSet) {
            CtClass inlineClass = Config.classPool.get(fullClassName)
            CtClass inlinePatchClass = PatchesFactory.cloneClass(inlineClass,
                NameManger.getInstance().getInlinePatchName(inlineClass.name), null)
     ...
     * @return
     */
    Set getAllInlineClasses(Set usedClass, Set patchMethodSignureSet) {
    ...
    static void dealInLineClass(String patchPath, List list) {
     ...
     */
    HashSet initInLineClass(Set classNamesSet, Set patchMethodSignureSet) {
        ...
        for (String fullClassName : classNamesSet) {
            ...
            modifiedCtclass.declaredMethods.each { method ->
                //找出modifiedclass中所有内联的类
                allPatchMethodSignureSet.addAll(
                    classInLineMethodsMap.getOrDefault(fullClassName, new ArrayList()))
                if (isNewClass || allPatchMethodSignureSet.contains(method.longName)) {
                    //                        isNewClass=false;
                    method.instrument(new ExprEditor() {
                        @Override
                        void edit(MethodCall m) throws CannotCompileException {
                            List inLineMethodList = classInLineMethodsMap.getOrDefault(
                                m.method.declaringClass.name, new ArrayList())
                            ClassMapping classMapping = ReadMapping.getInstance().
                                getClassMapping(m.method.declaringClass.name)
                            if (null != classMapping &&
                                classMapping.memberMapping.get(
                                    ReflectUtils.getJavaMethodSignureWithReturnType(m.method)) ==
                                null) {
                                ...
                                if (!inLineMethodList.contains(m.method.longName)) {
                                    ...
                                    classInLineMethodsMap.put(m.method.declaringClass.name,
                                        inLineMethodList)
                                }
                            }
                        }
    ...
    def replaceInlineMethod(MethodCall m, CtMethod method, boolean isNewClass) {
        ClassMapping classMapping = ReadMapping.getInstance().
            getClassMapping(m.method.declaringClass.name)
        if (null != classMapping &&
            classMapping.memberMapping.get(
                ReflectUtils.getJavaMethodSignureWithReturnType(m.method)) ==
            null) {
            m.replace(ReflectUtils.getInLineMemberString(m.method,
                ReflectUtils.isStatic(method.modifiers), isNewClass))
            return true
        }
        return false
    }

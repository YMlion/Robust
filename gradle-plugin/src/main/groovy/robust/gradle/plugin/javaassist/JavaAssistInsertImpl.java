...
public class JavaAssistInsertImpl extends InsertCodeStrategy {
    ...
    public JavaAssistInsertImpl(List<String> hotfixPackageList, List<String> hotfixMethodList,
                ...
                for (CtBehavior ctBehavior : ctClass.getDeclaredBehaviors()) {
                    ...
                    try {
                        if (ctBehavior.getMethodInfo().isMethod()) {
                            ...
                            body +=
                                    "   if (com.meituan.robust.PatchProxy.isSupport($args, argThis, "
                                            + Constants.INSERT_FIELD_NAME
                                            + ", "
                                            + isStatic
                                            + ", "
                                            + methodMap.get(ctBehavior.getLongName())
                                            + ","
                                            + parametersClassType
                                            + ","
                                            + returnTypeString
    ...
    private boolean isQualifiedMethod(CtBehavior it) throws CannotCompileException {
        ...
        if ((it.getModifiers() & AccessFlag.SYNTHETIC) != 0 && !AccessFlag
                .isPrivate(it.getModifiers())) {
            return false;
        }
    ...
    private String getReturnStatement(String type, boolean isStatic, int methodNumber,
            String parametersClassType, String returnTypeString) {
        switch (type) {
            case Constants.CONSTRUCTOR:
                return "    com.meituan.robust.PatchProxy.accessDispatchVoid( $args, argThis, changeQuickRedirect, "
                        + isStatic
                        + ", "
                        + methodNumber
                        + ","
            ...
            case Constants.LONG:
                return "   return ((java.lang.Long)com.meituan.robust.PatchProxy.accessDispatch( $args, argThis, changeQuickRedirect, "
                        + isStatic
                        + ","
                        + methodNumber
                        + ","

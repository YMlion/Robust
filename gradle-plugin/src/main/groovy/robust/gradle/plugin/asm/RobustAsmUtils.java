...
public final class RobustAsmUtils {
    ...
    public final static String REDIRECT_FIELD_NAME = "changeQuickRedirect";
    public final static String REDIRECT_CLASS_NAME =
            Type.getDescriptor(com.meituan.robust.ChangeQuickRedirect.class);
    public final static String PROXY_CLASS_NAME = "com.meituan.robust.PatchProxy".replace(".", "/");

    /**
     * 插入代码
     */
    public static void createInsertCode(GeneratorAdapter mv, String className, List<Type> args,
            Type returnType, boolean isStatic, int methodId) {
        prepareMethodParameters(mv, className, args, returnType, isStatic, methodId);
        //开始调用
        mv.visitMethodInsn(Opcodes.INVOKESTATIC, PROXY_CLASS_NAME, "proxy",
        ...
        mv.visitFieldInsn(Opcodes.GETFIELD, "com/meituan/robust/PatchProxyResult", "isSupported",
                          "Z");
        ...
        if ("V".equals(returnType.getDescriptor())) {
            ...
            mv.loadLocal(local);
            mv.visitFieldInsn(Opcodes.GETFIELD, "com/meituan/robust/PatchProxyResult", "result",
                              "Ljava/lang/Object;");
            //强制转化类型
    ...
    private static void prepareMethodParameters(GeneratorAdapter mv, String className,
        ...
        mv.visitFieldInsn(Opcodes.GETSTATIC, className, REDIRECT_FIELD_NAME, REDIRECT_CLASS_NAME);
    ...
    protected static void redirectLocal(GeneratorAdapter mv, Type arg) {
        switch (arg.getDescriptor()) {
            case "Z":
                mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/Boolean", "TYPE",
                                  "Ljava/lang/Class;");
                break;
            case "B":
                ...
                break;
            case "C":
                mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/Character", "TYPE",
                                  "Ljava/lang/Class;");
                break;
            case "S":
            ...
            case "F":
                ...
                break;
            case "D":
                mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/Double", "TYPE",
                                  "Ljava/lang/Class;");
                break;
            case "J":
     ...
     * 创建局部参数代码
     */
    private static void createObjectArray(MethodVisitor mv, List<Type> paramsTypeClass,
            boolean isStatic) {
        ...
        for (int i = 0; i < argsCount; i++) {
            ...
            if (i >= 1) {
                ...
                if ("J".equals(paramsTypeClass.get(i - 1).getDescriptor()) || "D"
                        .equals(paramsTypeClass.get(i - 1).getDescriptor())) {
    ...
    private static void createShortObj(MethodVisitor mv, int argsPosition) {
    ...
    private static void createCharObj(MethodVisitor mv, int argsPosition) {
    ...
    private static void createIntegerObj(MethodVisitor mv, int argsPosition) {
    ...
    private static void createFloatObj(MethodVisitor mv, int argsPosition) {
        mv.visitTypeInsn(Opcodes.NEW, "java/lang/Float");
        mv.visitInsn(Opcodes.DUP);
        mv.visitVarInsn(Opcodes.FLOAD, argsPosition);
        mv.visitMethodInsn(Opcodes.INVOKESPECIAL, "java/lang/Float", "<init>", "(F)V");
        mv.visitInsn(Opcodes.AASTORE);
    }

    private static void createDoubleObj(MethodVisitor mv, int argsPosition) {
        mv.visitTypeInsn(Opcodes.NEW, "java/lang/Double");
        mv.visitInsn(Opcodes.DUP);
        mv.visitVarInsn(Opcodes.DLOAD, argsPosition);
        mv.visitMethodInsn(Opcodes.INVOKESPECIAL, "java/lang/Double", "<init>", "(D)V");
        mv.visitInsn(Opcodes.AASTORE);
    }

    private static void createLongObj(MethodVisitor mv, int argsPosition) {
        mv.visitTypeInsn(Opcodes.NEW, "java/lang/Long");
        mv.visitInsn(Opcodes.DUP);
        mv.visitVarInsn(Opcodes.LLOAD, argsPosition);
    ...
    private static boolean createPrimateTypeObj(MethodVisitor mv, int argsPosition, String typeS) {
        ...
        if ("C".equals(typeS)) {
            createCharObj(mv, argsPosition);
            return true;
        }
        if ("S".equals(typeS)) {
            createShortObj(mv, argsPosition);
            return true;
        }
        if ("I".equals(typeS)) {
            createIntegerObj(mv, argsPosition);
            return true;
        }
        if ("F".equals(typeS)) {
            createFloatObj(mv, argsPosition);
            return true;
        }
        if ("D".equals(typeS)) {
            createDoubleObj(mv, argsPosition);
            return true;
        }
        if ("J".equals(typeS)) {
            createLongObj(mv, argsPosition);
            return true;
        }
        return false;
    }
